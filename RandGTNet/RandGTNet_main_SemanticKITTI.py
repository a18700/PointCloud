...
from RandGTNet import Network, compute_loss, compute_acc, IoUCalculator
...
print(len(TRAIN_DATASET), len(TEST_DATASET)) # 19130, 4071
TRAIN_DATALOADER = DataLoader(TRAIN_DATASET, batch_size=cfg.batch_size, shuffle=True, num_workers=4, worker_init_fn=my_worker_init_fn, collate_fn=TRAIN_DATASET.collate_fn)
TEST_DATALOADER = DataLoader(TEST_DATASET, batch_size=cfg.batch_size, shuffle=True, num_workers=4, worker_init_fn=my_worker_init_fn, collate_fn=TEST_DATASET.collate_fn)

print(len(TRAIN_DATALOADER), len(TEST_DATALOADER)) # 9565, 2036
        ...
        for key in batch_data:
            ...
            if type(batch_data[key]) is list:
                for i in range(len(batch_data[key])):
                    ...
                    print("{} : {}".format(i, batch_data[key][i]))
            else:
                batch_data[key] = batch_data[key].cuda()

        '''
        print(batch_data['xyz'][0].shape)
        print(batch_data['neigh_idx'][0].shape)
        print(batch_data['sub_idx'][0].shape)
        print(batch_data['interp_idx'][0].shape)
        print(batch_data['features'].shape)
        print(batch_data['labels'].shape)
        print(batch_data['input_inds'].shape)
        print(batch_data['cloud_inds'].shape)
        print(batch_data)
        assert 1==2, "check"

        torch.Size([2, 45056, 3])
        torch.Size([2, 45056, 16])
        torch.Size([2, 11264, 16])
        torch.Size([2, 45056, 1])
        torch.Size([2, 3, 45056])
        torch.Size([2, 45056])
        torch.Size([2, 45056])
        torch.Size([2, 1])
        '''
