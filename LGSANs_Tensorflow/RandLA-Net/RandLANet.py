...
class Network:
    ...
    def gt_res_block(self, feature, xyz, neigh_idx, d_out, name, is_training):
        ...
        f_pc = helper_tf_util.conv2d(feature, d_out // 2, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        f_pc = self.gtmodule(xyz, f_pc, neigh_idx, d_out, name + 'LFA', is_training)
        f_pc = helper_tf_util.conv2d(f_pc, d_out * 2, [1, 1], name + 'mlp2', [1, 1], 'VALID', True, is_training,
                                     activation_fn=None)
        shortcut = helper_tf_util.conv2d(feature, d_out * 2, [1, 1], name + 'shortcut', [1, 1], 'VALID',
                                         activation_fn=None, bn=True, is_training=is_training)
    ...
    def gtmodule(self, xyz, feature, neigh_idx, d_out, name, is_training, first=None):
        ...
        localvalue, f_xyz = group_gather_neighbour(localvalue, neigh_idx, None, d_in//g) # b,g,n,k,c//g & b,g,n,k,3
        ...
        f_xyz = helper_tf_util.conv2d(f_xyz, d_out//2, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training, groups=g) # b,n,k,d_out//2
        f_xyz = tf.reshape(f_xyz, [b, n, k, g, (d_out//2)//g]) #b,n,k,g,d_out//2*g
        #f_xyz = tf.transpose(f_xyz, perm=(0,3,4,))
        
        f1 = tf.concat([localvalue, tf.reshape(f_xyz, [b, g, d_in//g, n, k])], dim=2) # b,n,k,g,c//g+d_out//2*g 
        v1, attention_centrality = self.gt_pooling(f1, d_out//2, name + 'gt_pooling_1', is_training)
    ...
    def relative_pos_encoding(self, xyz, neighbor_xyz):
        ...
        print("  check rel pos")
        print("  xyz", xyz.get_shape()) # ?, ?, 3
        #print("  f_xyz", f_xyz.get_shape()) # ?, ?, 1, c # bgnk3?
        print("  k", k) # ?, ?, 1, c
        #print("  d_in 1", d_in) # 1, c
        
        #xyz_tile = tf.tile(tf.expand_dims(xyz, axis=2), [1,1,1,k,1]) # b,g,n,1,3
        #xyz = tf.repeat(xyz, groups, axis=3) # b,g,n,k,3

        #xyz_tile = tf.tile(tf.expand_dims(xyz, axis=2), [1, 1, tf.shape(neigh_idx)[-1], 1]) 
        
        xyz_tile = tf.tile(tf.expand_dims(xyz, axis=1), [1,g,1,1]) # b,1,n,3 -> b,g,n,3
        xyz_tile = tf.tile(tf.expand_dims(xyz_tile, axis=3), [1,1,1,k,1]) # bgn13 -> bgnk3
    ...
    @staticmethod
    def group_gather_neighbour(pc, neighbor_idx, cq, cv):
        # xyz : pyt = b, n, 3  & tf = b, n, 3 
        ...
        for i in range(g):
            ...
            feature = gather_neighbour(g_pc, g_neighbor_idx, idx_base)
    ...
    def gt_pooling(feature_set, d_out, name, is_training, neigh_idx):
        ...
        att_activation = tf.layers.dense(f_reshaped, d, activation=None, use_bias=False, name=name + 'fc') # b*n*g, k, d
        att_scores = tf.nn.softmax(att_activation, axis=1) # b*n*g, k, d, softmaxed over k
