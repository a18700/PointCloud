...
class Network:
    ...
    def gt_res_block(self, feature, xyz, neigh_idx, d_out, name, is_training, depth):
    ...
    def group_gather_neighbour(self, pc, neighbor_idx, cq, cv):
        ...
        value = f_concat[:,:,:,:,:cv]
        xyz = f_concat[:,:,:,:,-3:]
        #print("value", f_concat.get_shape())
        #print("xyz", f_concat.get_shape())
    ...
    def gather_neighbour(self, pc, neighbor_idx):
        ...
        features = tf.reshape(features, [self.b, n, tf.shape(neighbor_idx)[-1], d]) # b, n, k, d
        return features
