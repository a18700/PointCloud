...
class Network:
    def __init__(self, dataset, config):
        ...
        self.knl = config.num_nonlocal_neighbors
    ...
    def inference(self, inputs, is_training):
        ...
        for i in range(self.config.num_layers):
            ...
            #TODO CHECK PLZ
            s_encoder_i, f_encoder_i = self.gt_res_block(feature, inputs['xyz'][i], inputs['neigh_idx'][i], d_out[i],'Encoder_layer_' + str(i), is_training)
    ...
    def gt_res_block(self, feature, xyz, neigh_idx, d_out, name, is_training):
        ...
        f_pc = helper_tf_util.conv2d(feature, d_out // 2, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        stored_f_pc, f_pc = self.gtmodule(xyz, f_pc, neigh_idx, d_out, name + 'LFA', is_training)
        ...
        return tf.nn.leaky_relu(stored_f_pc + shortcut), tf.nn.leaky_relu(f_pc + shortcut)
    ...
    def gtmodule(self, xyz, feature, neigh_idx, d_out, name, is_training, first=None):
        #TODO CHECK PLZ
        # xyz : pyt = b, n, 3  & tf = b, n, 3 
        # feature : pyt = b, c, n, 1 & tf = b, n, 1, c
        # neigh_idx : pyt = b, n, k & tf = b, n, k
        ...
        # Stage1 : Local Layer
        localvalue = tf.reshape(feature, [-1, self.n, self.g, d_in//self.g]) # b,n,g,c//g
        ...
        f_xyz = tf.reshape(f_xyz, [self.b, self.n, self.k, -1]) # b,n,k,g*10
        ...
        f_xyz = tf.reshape(f_xyz, [self.b, self.n, self.k, self.g, ld_out//self.g]) #b,n,k,g,d_out//2*g
        
        localvalue = tf.concat([localvalue, f_xyz], axis=-1) # b,n,k,g,c//g+d_out//2*g 
        localvalue, attention_centrality = self.gt_pooling(localvalue, ld_out, name + 'gt_pooling_1', is_training, neigh_idx) 
        ...
        ac_idx = tf.tile(tf.expand_dims(ac_idx, axis=2), [1,1,self.n,1]) # bgk -> bgnk
        ...
        g_xyz = tf.reshape(g_xyz, [self.b, self.n, self.k, -1]) # b,n,k,g*d
        ...
        g_xyz = helper_tf_util.grouped_conv2d(name + 'global_pe', 
                                     ...
                                     batchnorm_enabled=True, 
                                     is_training=is_training)
        g_xyz = tf.reshape(g_xyz, [self.b, self.n, self.k, self.g, nld_out//self.g]) #b,n,k,g,d_out//2*g
        globalvalue = tf.concat([globalvalue, g_xyz], axis=-1) # b,n,k,g,c//g+d_out//2*g 
        
        # Gating
        globalvalue, attention_centrality = self.gt_pooling(globalvalue, nld_out, name + 'gt_pooling_2', is_training, neigh_idx) 
        
        globalvalue = globalvalue * tf.math.tanh(tf.expand_dims(tf.transpose(attention_centrality, [0,2,1]), axis=-1)) #bngc, bng1
        
        # Cat Two
        localvalue = tf.reshape(localvalue, [self.b, self.n, 1, -1])
        globalvalue = tf.reshape(localvalue, [self.b, self.n, 1, -1])
    ...
    def group_gather_neighbour(self, pc, neighbor_idx, cq, cv):
        #TODO CHECK PLZ
        # xyz : pyt = b, n, 3  & tf = b, n, 3 
        # neigh_idx : pyt = b, g, n, k & tf = b, g, n, k
        
        features = []

        for i in range(self.g):
            g_pc = pc[:,i,:,:]
            g_neighbor_idx = neighbor_idx[:,i,:,:]
            feature = self.gather_neighbour(g_pc, g_neighbor_idx)
            feature = tf.expand_dims(feature, axis=1) # b,1,n,k,d
    ...
    def gather_neighbour(self, pc, neighbor_idx):
        ...
        index_input = tf.reshape(neighbor_idx, shape=[self.b, -1])
        features = tf.batch_gather(pc, index_input) 
        features = tf.reshape(features, [self.b, self.n, tf.shape(neighbor_idx)[-1], d]) # b, n, k, d
        return features
    ...
    def gt_pooling(self, feature_set, d_out, name, is_training, neigh_idx):
        ...
        feature_set = tf.reshape(feature_set, [self.b, self.n, self.k, -1]) # b,n,k,g*10
        ...
        att_activation = tf.reshape(att_activation, [self.b, self.n, self.k, self.g, d])
    ...
    def attention_centrality(self, attention, idx):
        ...
        centrality = tf.zeros([self.b, self.g, self.n], tf.float32)
        idx = tf.reshape(idx, [self.b, self.n, -1])
        attention = tf.reshape(attention, [self.b, self.n, -1]) # b,g,n
        #centrality = __tf_scatter_add(attention, idx, centrality)
        centrality = self._tf_scatter_add(centrality, idx, attention)
        return centrality
