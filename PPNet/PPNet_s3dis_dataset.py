...
from PPNet_helper_tool import DataProcessing as DP
from PPNet_helper_ply import read_ply
...
class S3DIS:
    ...
    def np_map(self, batch_xyz, batch_features, batch_labels, batch_pc_idx, batch_cloud_idx):
        ...
        # PPNet implementation required

        #neigh idx : torch.Size([2, 40960, 16])
        #neigh idx : torch.Size([2, 10240, 16])
        #neigh idx : torch.Size([2, 2560, 16])
        #neigh idx : torch.Size([2, 640, 16])
        #neigh idx : torch.Size([2, 160, 16])
        ...
        mask = np.ones([batch_features.shape[0], batch_features.shape[1]])

        input_list = input_points + input_neighbors + input_pools + input_up_samples
        input_list += [batch_features, batch_labels, batch_pc_idx, batch_cloud_idx, mask]
    ...
    def collate_fn(self, batch):
        ...
        # PPNet implementation required
        inputs['mask'] = torch.from_numpy(flat_inputs[4 * num_layers + 4]).type(torch.int32)
