...
class GTResNet(nn.Module):
    ...
    def forward(self, end_points):
        ...
        # res5
        xyz, mask, features, attention_centrality = self.layer4(xyz, mask, features, None, end_points['neigh_idx'][4])
        end_points['res5_xyz'] = xyz
        end_points['res5_mask'] = mask
        end_points['res5_features'] = features
...
class GTBottleneck(nn.Module):
    ...
    def forward(self, xyz, mask, features, attention_centrality=None, idx_knn=None):
        ...
        if self.downsample:
            #MEMCHK# 0
            tic = time.time()
            cnt, tic = memchk(cnt, level=4, tic=tic, string="Before downsample")
            #MEMCHK#
            sub_xyz, sub_mask, sub_features = self.maxpool(xyz, mask, features)
            query_xyz = sub_xyz
            query_mask = sub_mask
            ...
            #MEMCHK# 0
            tic = time.time()
            cnt, tic = memchk(cnt, level=4, tic=tic, string="After downsample")
            #MEMCHK#
            
            #MEMCHK# 0
            tic = time.time()
            cnt, tic = memchk(cnt, level=4, tic=tic, string="Before conv")
            #MEMCHK#
            queryandkey = self.query(sub_features) # B, C, N
            value = self.value(sub_features) # B, C, N
            #MEMCHK# 0
            tic = time.time()
            cnt, tic = memchk(cnt, level=4, tic=tic, string="After conv")
            #MEMCHK#
...
class GTLModuleV1(nn.Module):
    ...
    def gather_neighbour(self, pc, neighbor_idx):  # pc: batch*npoint*channel
        ...
        features = torch.gather(pc, 1, index_input.unsqueeze(-1).repeat(1, 1, d)) # b,n,c -> b,nk,c
    ...
    def batch_gather_neighbour(self, pc, neighbor_idx):  
        ...
        features = torch.gather(pc, 1, index_input.unsqueeze(-1).repeat(1, 1, d)) # bg,n,c -> bg,nk,c
