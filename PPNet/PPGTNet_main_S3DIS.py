from PPNet_helper_tool import ConfigS3DIS as cfg
from PPNet import build_scene_segmentation, compute_loss, compute_acc, IoUCalculator
from PPNet_s3dis_dataset import S3DIS
import numpy as np
import os, argparse
#from utils.PPNet_config import config, update_config
...
TRAIN_DATASET = S3DIS('training', 5)
TEST_DATASET = S3DIS('validation', 5)
...
def train_one_epoch():
	...
	torch.Size([2, 40960, 3])
	torch.Size([2, 40960, 16])
	torch.Size([2, 10240, 16])
	torch.Size([2, 40960, 1])
	torch.Size([2, 6, 40960])
	torch.Size([2, 40960])
	torch.Size([2, 40960])
	torch.Size([2, 1])
        '''
